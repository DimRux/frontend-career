...
import { CARD_FOR_PAGE_DETAIL } from "../constants";
import {
  parseResultVacancy,
  schemeResultVacancy,
...
export const useVacancyLocal = create((set) => ({
  ...
  close: () => {
    set({ isOpen: false, vacancy: null, vacanciesList: [], pageDetail: 0 });
  },
  ...
  fetchVacancies: async (id, page = 0) => {
    try {
      ...
      const response = await fetch(
        `https://api.hh.ru/vacancies/${id}/similar_vacancies?order_by=publication_time&only_with_salary=true&per_page=${CARD_FOR_PAGE_DETAIL}&page=${page}`
      );
      if (!response.ok)
